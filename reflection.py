...
def _train_memory_factory(memory_layer: str, id_list: List[int]):
    class Memory(BaseModel):
        ...
        @validator('memory_index')
        def validate_memory_index(cls, v):
            if v == -1:  # Allow placeholder value
                return v
            if v not in id_list:
                raise ValueError(f"memory_index must be one of {id_list}")
            return v
...
def _test_reflection_factory(
    ...
    class InvestInfo(BaseModel):
        ...
        @validator('investment_decision')
        def validate_investment_decision(cls, v):
            if v not in ["buy", "sell", "hold"]:
                raise ValueError("investment_decision must be one of ['buy', 'sell', 'hold']")
            return v
        summary_reason: str = Field(
            ...,
...
def trading_reflection(
    ...
    try:
        ...
        raw_response = endpoint_func(prompt)
        ...
        try:
            ...
            if json_match:
                try:
                    ...
                    logger.error(f"Could not parse JSON from response: {raw_response}")
                    if run_mode == RunMode.Train:
                        return {
                            "summary_reason": "Unable to parse LLM response",
                ...
                if run_mode == RunMode.Train:
                    return {
                        "summary_reason": "No valid JSON in response",
                        "short_memory_index": [{"memory_index": -1}],
                        "middle_memory_index": [{"memory_index": -1}],
                        "long_memory_index": [{"memory_index": -1}],
                        "reflection_memory_index": [{"memory_index": -1}]
        ...
        try:
            validated_output = response_model(**parsed_json)
            return _delete_placeholder_info(validated_output.dict())
        except Exception as e:
            logger.error(f"Validation error: {str(e)}")
            if run_mode == RunMode.Train:
                return {
                    "summary_reason": f"Validation error: {str(e)}",
                    "short_memory_index": [{"memory_index": -1}],
                    "middle_memory_index": [{"memory_index": -1}],
                    "long_memory_index": [{"memory_index": -1}],
                    "reflection_memory_index": [{"memory_index": -1}]
