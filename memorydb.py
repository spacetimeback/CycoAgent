...
from .embedding import NuwaLongerThanContextEmb
...
class MemoryDB:  # can possibly take multiple symbols
    def __init__(
        ...
        self.emb_func = NuwaLongerThanContextEmb(**self.emb_config)
    ...
    def add_memory(self, symbol: str, date: date, text: Union[List[str], str]) -> None:
        ...
        if not text:
            self.logger.info("Empty text list provided, skipping memory addition")
            return
        ...
        if emb.size == 0:
            self.logger.info("Empty embedding generated, skipping memory addition")
            return
            
        # Convert embedding to float32 and ensure it's contiguous
    ...
    def save_checkpoint(self, name: str, path: str, force: bool = False) -> None:
        ...
        state_dict = {
            ...
            "emb_config": dict(self.emb_config),  # Convert TOML dict to regular dict
            ...
            "clean_up_threshold_dict": dict(self.clean_up_threshold_dict),  # Convert TOML dict to regular dict
            "logger": self.logger,
        }
